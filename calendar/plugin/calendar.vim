...
command! -nargs=* LunarDay  call LunarDay()
...
let s:Animals=["鼠","牛","虎","兔","龍","蛇","馬","羊","猴","雞","狗","豬"]
let s:solarTerm = ["小寒","大寒","立春","雨水","驚蟄","春分","清明","穀雨","立夏","小滿","芒種","夏至","小暑","大暑","立秋","處暑","白露","秋分","寒露","霜降","立冬","小雪","大雪","冬至"]
...
" 農曆節日 *表示放假日
let lFtv =[ "0101*春節", "0102*初二", "0115 元宵節", "0505*端午節", "0707 七夕情人節", "0715 中元節", "0815*中秋節", "0909 重陽節", "1208 臘八節", "1223 小年", "0100*除夕"] 
...
function! s:Lunar(y,m,d) 
    ...
    let nongliYear = i                                 " 農曆那一年

    let leap = s:GetLeapMonth(i)  " //閏哪個月
    let isLeap = 0
    let i = 1
    while  i < 13 && offset > 0
...
" return the current lunar day
function! LunarDay()
    let year = strftime('%Y')
    let month = strftime('%m')
    let day = strftime('%d')
    "echo s:Lunar(year,month,day)
    return s:Lunar(year,month,day)
endfunc
